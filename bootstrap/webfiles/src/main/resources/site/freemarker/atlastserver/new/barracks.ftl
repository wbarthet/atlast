...
<#-- @ftlvariable name="barracks" type="org.atlast.beans.Barracks" -->
...
<div class="section-headline">
  <div class="row">
    <div class="small-12 medium-12 large-12 large-centered columns">
      <h1>
        <img class="land-detail-icon" src="<@hst.link path="/binaries/content/gallery/atlastserver/icons/council/barracks.png"/>"/>Barracks

        <img class="land-detail-icon" src="<@hst.link path="/binaries/content/gallery/atlastserver/icons/council/barracks.png"/>"/>
    ...
    <form method="post" action="<@hst.actionURL/>">
      <p>
        ...
        <#list recipes as recipe>
          <#if barracks.recipeDescriptor.uuid == recipe.uuid>
        ...
        Wages: <input name="wages" class="input-number" type="number" step="0.01" value="${barracks.wages}">

        <input type="hidden" name="uuid" value="${barracks.uuid}">
        <input type="hidden" name="action" value="save">
        <input class="button round button-small" type="submit" value="save">
      </p>

    <#if barracks.pops?has_content>
    <#assign popcount = barracks.pops?size/>
    <#else>
      <#assign popcount = 0/>
    </#if>
      <p>
        Skill level ${barracks.getSkillLevel(barracks.recipeDescriptor.skill)}%
      </p>
      <p>
        <#assign pnl=0.00/>
        <#if barracks.recipeDescriptor.inputs?has_content>
          Input:
          <#list barracks.recipeDescriptor.inputs as input>
            <#assign inputCount = input.quantity + input.quantity * popcount / barracks.recipeDescriptor.labour/>
          ...
          |</#if>
        <#assign pnl = pnl - (barracks.wages * popcount)/>

        Labour: ${barracks.recipeDescriptor.labour} x
        <img style="width: 1.2em;" class="resource-icon" src="<@hst.link path="/binaries/content/gallery/atlastserver/img/upper.png"/>"/> |
        <#if barracks.recipeDescriptor.outputs?has_content>
          Output:
          <#list barracks.recipeDescriptor.outputs as output>
          ${output.resourceDescriptor.name} x ${barracks.outputs[output.resourceDescriptor.name]}
