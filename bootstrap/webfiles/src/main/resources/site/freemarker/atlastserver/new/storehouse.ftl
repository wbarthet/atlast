...
<div class="section-headline">
  <div class="row">
    <div class="small-12 medium-12 large-12 large-centered columns">
      <h1>
        <img class="land-detail-icon" src="<@hst.link path="/binaries/content/gallery/atlastserver/icons/council/storehouse.png"/>"/>Storehouse

        <img class="land-detail-icon" src="<@hst.link path="/binaries/content/gallery/atlastserver/icons/council/storehouse.png"/>"/>
  ...
  <div class="row">
    ...
    <form method="post" action="<@hst.actionURL/>">
      <p>
        ...
        <#if storehouse.recipeDescriptor??>
        ...
        <#list recipes as recipe>
          <#if storehouse.recipeDescriptor.uuid == recipe.uuid>
        ...
        Wages: <input name="wages" class="input-number" type="number" step="0.01" value="${storehouse.wages}">

        <input type="hidden" name="uuid" value="${storehouse.uuid}">
        <input type="hidden" name="action" value="save">
        <input class="button round button-small" type="submit" value="save">
      </p>

    <#if storehouse.pops?has_content>
    <#assign popcount = storehouse.pops?size/>
    <#else>
      <#assign popcount = 0/>
    </#if>
      <p>
        Skill level ${storehouse.getSkillLevel(storehouse.recipeDescriptor.skill)}%
      </p>
      <p>
        <#assign pnl=0.00/>
        <#if storehouse.recipeDescriptor.inputs?has_content>
          Input:
          <#list storehouse.recipeDescriptor.inputs as input>
            <#assign inputCount = input.quantity + input.quantity * popcount / storehouse.recipeDescriptor.labour/>
          ...
          |</#if>
        <#assign pnl = pnl - (storehouse.wages * popcount)/>

        Labour: ${storehouse.recipeDescriptor.labour} x
        <img style="width: 1.2em;" class="resource-icon" src="img/other/lower.png"/> |
        <#if storehouse.recipeDescriptor.outputs?has_content>
          Output:
          <#list storehouse.recipeDescriptor.outputs as output>
          ${output.resourceDescriptor.name} x ${storehouse.outputs[output.resourceDescriptor.name]}
